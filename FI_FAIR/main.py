...
if __name__ == '__main__':
    ...
    ## Importation of data and generation of "agent" instances with it. 

    # Bioconductor
    if config.BIOCONTUCTOR_TOOLS:
        bioconductor2000raw = FI.loadJSON(config.BIOCONTUCTOR_TOOLS)
        bioconductInts = FI.bioconductorAgentsGenerator(bioconductor2000raw).instSet
        allSets.append(FI.bioconductorAgentsGenerator(bioconductor2000raw).instSet)

    # bioconda
    if config.BIOCONDA_TOOLS:
        bioconda2000raw = FI.loadJSON(config.BIOCONDA_TOOLS)
        biocondaInts = FI.biocondaAgentsGenerator(bioconda2000raw).instSet
        allSets.append(FI.biocondaAgentsGenerator(bioconda2000raw).instSet)

    # bioagents
    if config.BIOTOOLS_TOOLS:
        bioagents2000raw = FI.loadJSON(config.BIOTOOLS_TOOLS)
        biotoInts = FI.bioagentsAgentsGenerator(bioagents2000raw).instSet
        allSets.append(FI.bioagentsAgentsGenerator(bioagents2000raw).instSet)

    # galaxyConfig
    if config.GALAXY_XML_TOOLS:
        shedXML2000raw = FI.loadJSON(config.GALAXY_XML_TOOLS)
        shedXMLInts = FI.galaxyConfigAgentsGenerator(shedXML2000raw).instSet
        allSets.append(FI.galaxyConfigAgentsGenerator(shedXML2000raw).instSet)

    # galaxy
    if config.GALAXY_TOOLS:
        shed2000raw = FI.loadJSON(config.GALAXY_TOOLS)
        shedInts = FI.galaxyShedAgentsGenerator(shed2000raw).instSet
        allSets.append(FI.galaxyShedAgentsGenerator(shed2000raw).instSet)
     

    ## Integration of "agent" instances of same ID (name, version, type)
    ...
    ## Exportation of integrated agents to file
 ...
 ##### ---------------- FAIRness calculation---------------------------------------------------#########################
    ...
    if config.SCORES_CALC == True:
        # canonicals FAIRness
        print("Calculating canonical agents FAIRsoft metrics and scores ...")
