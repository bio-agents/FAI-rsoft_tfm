...
class instance(object):
    ...
    def __init__(self, name, type_, version):
        ...
        self.input = [] # list of dictionaries bioagents-like {'format' : <format> , 'uri' : <uri> , 'data' : <data> , 'uri': <uri>}
        self.output = [] # list of dictionaries bioagents-like {'format' : <format> , 'uri' : <uri> }
    ...
    global struct_meta
    struct_meta = ['bioagents', 'bioconda', 'bioconductor', 'galaxyShed', 'galaxyConfig']
    def compF2_1(self):
        '''
...
class agentGenerator(object):
    def __init__(self, agents, source):
        self.agents = agents
        self.source = source
...
class biocondaAgentsGenerator(agentGenerator):
    def __init__(self, agents, source = 'bioconda'):
        ...
        for agent in self.agents:
            ...
            type_ = 'Command-line agent' # all biocondas are cmd
            ...
            if agent['web']['homepage']:
                ...
                newInst.links = []
            newInst.publication =  len(agent['publications']) # number of related publications [by now, for simplicity]
            newInst.download = [ [ down ,agent['distributions'][down] ] for down in agent['distributions'].keys()  ]  #
    ...
    def __init__(self, agents, source='bioconductor'):
        ...
        for agent in self.agents:
            ...
            if agent['URL']:
                ...
                newInst.links = []
            newInst.publication =  int(agent['citation']) # number of related publications [by now, for simplicity]
            download = []
            for a in ["Windows Binary", "Source Package", "Mac OS X 10 10.11 (El Capitan)"]:
                if a in agent.keys() and agent[a]:
                    download.append([a, agent['Package Short Url'] + agent[a]])
...
class galaxyConfigAgentsGenerator(agentGenerator):
    def __init__(self, agents, source = 'galaxyConfig'):
        agentGenerator.__init__(self, agents, source)
        ...
        for agent in self.agents:
            ...
            if agent['ciation']:
                newInst.publication =  len(agent['ciation']) # number of related publications [by now, for simplicity]
...
def integrateInstances(setsOfInsts):
    ...
    #print('There are %d agent names'%len(totalNames))

    # Grouping the instances by name and type in a dictionary
    count = 0
    Count = 0
    for name in totalNames: # iterating agent names
        Count += 1
...
def generateCanonicalSet(instsctDist):
    ...
    for name in instsctDist.keys():
        ...
        newCanon = canonicalAgent(name, instances, sources, types)
